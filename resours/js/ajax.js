...
function ajax(option){
    ...
    // 【5】判断callback回调函数 
    if(!defOption.callback){
        throw new Error('callback是必须存在的参数')
    }

    // 判断callback 是否是函数
    if(Object.prototype.toString.call(defOption.callback) != '[object Function]'){
    ...
    if(defOption.async){
        xhr.onload = function(){
            defOption.callback(xhr.responseText)
        }
    }else{
        defOption.callback(xhr.responseText)
    } 
